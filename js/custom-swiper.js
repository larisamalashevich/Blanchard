...
function CustomSwiper(selector, props) {
  ...
  this.breakpoints = Object.entries(props.breakpoints)
    .concat([[-1, defaultSizes]])
    .map(([key, value]) => ({
      minWidth: +key,
      params: value,
    }))
  ...
  this.wrapperElement.addEventListener("pointerup", (event) =>
    this.onTouchEnd(event, "up")
  );
  this.wrapperElement.addEventListener("pointercancel", (event) =>
    this.onTouchEnd(event, "cansel")
  );
  // this.wrapperElement.addEventListener('pointerout' , (event) => event.target == this.element && this.onTouchEnd(event, 'out'))

  this.wrapperElement.addEventListener("pointermove", (event) =>
...
CustomSwiper.prototype.onTouchEnd = function (event, reason) {
  ...
  if (this.moveAnchor) {
    ...
    const columnWidth =
      this.wrapperElement.getBoundingClientRect().width / this.fullColumnsCount;

    if (
      columnWidth * this.sizes.slidesPerGroup * SWIPER_MULTIPLYER <
      Math.abs(this.moveAnchor.endPosition - this.moveAnchor.startPosition)
    ) {
      this.slidePage(
        (this.moveAnchor.endPosition > this.moveAnchor.startPosition ? -1 : 1) *
          Math.ceil(
            Math.abs(
              this.moveAnchor.endPosition - this.moveAnchor.startPosition
            ) /
              (columnWidth * this.sizes.slidesPerGroup)
          )
      );

      this.moveAnchor.startPosition = this.moveAnchor.endPosition;
      this.moveAnchor.baseWrapperTransform =
...
CustomSwiper.prototype.onTouchMove = throttle(function (event) {
  ...
  if (this.moveAnchor) {
    ...
    this.wrapperElement.style.transform =
      this.moveAnchor.baseWrapperTransform +
      ` translate3d(${currentPostion - this.moveAnchor.startPosition}px, 0, 0)`;
    this.moveAnchor.endPosition = currentPostion;
  }
}, 10);

CustomSwiper.prototype.onWindowResize = function () {
...
CustomSwiper.prototype.updateSizes = async function () {
  ...
  const slideHeight = `calc((100% - ${
    this.sizes.spaceBetween * (this.sizes.slidesPerColumn - 1)
  }px) / ${this.sizes.slidesPerColumn})`;
  const slideWidth = `calc((100% - ${
    this.sizes.spaceBetween * (this.fullColumnsCount - 1)
  }px) / ${this.fullColumnsCount})`;

  this.slides.forEach((slide, index) => {
    slide.style.marginLeft =
      index >= this.sizes.slidesPerColumn
        ? `${this.sizes.spaceBetween}px`
        : `0px`;
  ...
  this.wrapperElement.style.width = `calc(${100 * fullSlidesCount}% + ${
    (fullSlidesCount - 1) * this.sizes.spaceBetween
  }px)`;
  this.updatePosition();
...
CustomSwiper.prototype.scanNodes = function () {
  this.slides = [
    ...this.wrapperElement.querySelectorAll(this.slideSelector),
  ].filter((element) => !element.hidden);
};

CustomSwiper.prototype.moveTo = function (slide) {
  if (slide < 0) slide = 0;
  if (slide >= this.slides.length) slide = this.slides.length - 1;
...
CustomSwiper.prototype.getCurrentPage = function (slide = null) {
  ...
  return Math.trunc(
    slide / (this.sizes.slidesPerColumn * this.sizes.slidesPerGroup)
  );
};
CustomSwiper.prototype.updatePosition = function () {
  const currentColumn = this.getCurrentPage() * this.sizes.slidesPerGroup;

  const translate = `translate3d(calc((100% - ${
    (this.fullColumnsCount - 1) * this.sizes.spaceBetween
  }px) / ${-this.fullColumnsCount} * ${currentColumn} - ${
    currentColumn * this.sizes.spaceBetween
  }px), 0, 0  )`;
...
CustomSwiper.prototype.updatePaginator = function () {
  if (this.paginationElement)
    this.paginationElement.textContent = `${this.getCurrentPage() + 1} / ${
      this.pages
    }`;
};
...
CustomSwiper.prototype.slidePage = function (direction) {
  this.moveTo(
    this.slide +
      direction * this.sizes.slidesPerGroup * this.sizes.slidesPerColumn
  );
};
